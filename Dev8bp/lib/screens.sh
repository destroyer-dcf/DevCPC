...
convert_screens() {
    ...
    local img_tool="$DEV8BP_TOOLS/abasm/src/img.py"
    if [[ ! -f "$img_tool" ]]; then
...
add_screens_to_dsk() {
    ...
    for scn_file in "${scn_files[@]}"; do
        ...
        if python3 "$DEV8BP_TOOLS/abasm/src/dsk.py" "$DIST_DIR/$DSK" --put-bin "$scn_file" --load-addr 0xC000 --start-addr 0xC000 > /dev/null 2>&1; then
            ((added++))
        else
