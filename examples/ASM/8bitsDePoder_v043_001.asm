...
;   8888   BBBBBB   PPPP
;
; Jose Javier Garcia Aranda 2016-2023
;    Las limitaciones no son un problema. son una fuente de inspiracion
;
; CONJUNTO DE RUTINAS ASM PARA PROGRAMAS (JUEGOS) BASIC y C
; ------------------------------------------------------------------------------------------------------------
...
;|MOVER, #,dy,dx                    -> movimiento re lativo de un sprite
...
; 33600 +---------- 
;       | canciones (1.5 KB para musica editada con WYZtracker 2.0.1.0)       
;       |		       
; 32100 +----------
...
;
let capacidad_scrolling = 0
let capacidad_pseudo3d  = 0
let capacidad_layout  = 0
   
if ASSEMBLING_OPTION = 0  ; OPCION 0 TODO DISPONIBLE
  let capacidad_scrolling = 1
  let capacidad_pseudo3d  = 1
  let capacidad_layout  = 1
elseif ASSEMBLING_OPTION = 1 ; OPCION 1 JUEGOS LAYOUT
  let capacidad_scrolling = 0
  let capacidad_pseudo3d  = 0
  let capacidad_layout  = 1
elseif ASSEMBLING_OPTION = 2 ; OPCION 2 JUEGOS scroll
  let capacidad_scrolling = 1
  let capacidad_pseudo3d  = 0
  let capacidad_layout  = 0
elseif  ASSEMBLING_OPTION = 3 ; OPCION 3 JUEGOS pseudo3d
  let capacidad_scrolling = 1
  let capacidad_pseudo3d  = 1
  let capacidad_layout  = 0
elseif  ASSEMBLING_OPTION = 4 ; OPCION 3 JUEGOS sin scroll ni layout
  let capacidad_scrolling = 0
  let capacidad_pseudo3d  = 0
  let capacidad_layout  = 0
endif
...
; para la impresion DELETE
SPR_flag_delete db 0
...
;---------------------------
;hay mas variables de MAP2SP en otros segmentos
...
; DESPUES DE LOS SEGMENTOS PODEMOS PONER LOS IF 

if ASSEMBLING_OPTION = 0 ;
org 23600
endif ; end  OPTION 0 

if ASSEMBLING_OPTION = 1 ; juegos de layout
; aqui hay que definir direcciones de memoria y tambien variables
; para que no falle el ensamblaje del resto del codigo, aunque no se usen.
; comenzamos en 25000 !!!
  org 25000

  ; funciones que no deben hacer nada (y no se deberian invocar en assembling option =0)
  _AJUSTA_SEGMENTO	;option 3
  _3D_PROYECTA_XY	;option 3
  _3D_GET_ZOOM_IMAGE	;option 3
  _3D_PAINT_SEGMENT	;option 3
  _RENDER3D		;option 3
  _3D_RENDER3D 		;option 3
  _UPDATE_MAP		;option 2
  _MAP2SP		;option 2
			ret 

  ; variables que deben existir para que se ensamble sin errores
  R3D_mode db 0
  PNTS_pt2_clipvinf	db 0; 0=noclipvinf pt2 , 1=si clipvinf pt2
  PSTR_bin3		dw 0
  PSTR_bin1		dw 0
  R3D_MAX_sprite_Dir 	dw 25000; por defecto ninguno se proyecta. Serviria un 27000-1 pues es la dir del primer sprite 3D
...
if ASSEMBLING_OPTION = 2 ; scrolling
  ;org 25000
  org 24800
  ...
  _RENDER3D
  _3D_RENDER3D 		;option 3
  _PRINT_LAYOUT 	;option 1
  _COLISION_LAYOUT 	;option 1
...
if ASSEMBLING_OPTION = 3 ; scrolling + 3d
  org 24000; v33_013

  ; estas variables se definen al final para que no ocupen memoria aqui
  ; funciones que no deben hacer nada (y no se deberian invocar en assembling option =0)
  ;_PRINT_LAYOUT 	;option 0
  ;_COLISION_LAYOUT 	;option 0
			ret
  ; variables que deben existir para que se ensamble sin errores
endif

if ASSEMBLING_OPTION = 4 ; sin layout ni scroll (ni 3d)
 org 25300
...
;=================================================================================================================
;                               CODIGO RELACIONADO CON PSEUDO-3D   V42-3D
;                               ------------------------------------------
if capacidad_pseudo3d  = 1 ; 
;org 24000; v33_013
;============================================================================
...
; entra con bc =dir image y retorna con bc final
	...
	ld HL,_BEGIN_3D_ZOOM_IMAGES-1
	ld DE,_END_3D_ZOOM_IMAGES-1
	CALL CHECK_DOUBLE_LIMITS
	ret NC
...
_END_CODE_3D ; v42 para saber hasta donde podemos ampliar memoria si no se usa 3D

endif ; capacidad_pseudo3d
...
if  capacidad_scrolling  = 1; scrolling
...
;=================================================
; Tabla de alturas de sprites. esta tabla se ubica en la memoria de sprites (32 bytes en fichero images.asm)
;=================================================
...
_SET_DEFAULT_LIMITS
	...
	ld hl,LIMITS
	ld DE,LIMITS_BACKUP
	ld bc, 10;   en v42 me he dado cuenta de que esto tenia que ser 8
	LDIR
...
_SETcommon	ld bc, 10; 10 en v42 he cambiado el 10 por un 8
		LDIR ; (DE)<-(HL) till bc=0
		ret
...
;		jr m1PTR2_4
		...
		;and a,%11110000; 
		;and a,%00001111; 
		and a; ******* v41_003 esto antes era and a,%11110000; tb es parte de V42
...
;_AJUSTA_SEGMENTO ;  (AJS) V42 movido


;===============================================================================================
;_INSTALL_RSX2 V42 lo he quitado de aqui
...
PAT_over	ld (SPR_status),a
		...
		ld a, (HL)
		ld (LAY_len), A; longitud del string , por ejemplo "HOLA" es 4
		inc HL
		...
		CALL _SET_DEFAULT_LIMITS ;AQUI ESTA EL PROBLEMA!!!
		;CALL BG_SPRITES; v42 si se invoca con transparencia en printat, habria que invocar
...
PAT_NOT_found
		...
		dec c; ahora c contiene el indice del ultimo caracter en el alfabeto
		;or a; carry off
...
; me las llevo a la zona final de ensamblaje
...
PS2_noparam
		CALL _GET_MODE ; v34 obtiene el modo grafico. rellena la variable GM_mode
...
;_ME LLEVO MAP2SP a assembling mode v42
...
_PSP
PSP:; RUTINA PSP (usada desde printlayout) _PRINT_SPRITES usa psp_i
;		cp 3; v33
;		JR Z,PSP_xy; v33
...
;		JR Z,PSP_sp
		ld b,a
		CALL _GET_MODE; almacena el modo en la variable GM_mode
		ld a,b
		...
		;------------------ v33 2 param ----- ejemplo PRINTSP,32,2 o bien PRINTSP,32,1
		; para establecer 2 bit o 1 bit de fondo
		ld a,(IX+2)
		cp 33
		jp z, _SET_FLAG_DEL
		cp 32
...
; ***********************************a partir de aqui hay que borrar *************************
;		dec (IX+0);optimizacion. Esto tiene el numero de bits
;		
;		JR NZ, PSP_2bitTR
;PSP_1bitTR
;		ld a,%11000000
...
;		JR  PSP_confTR
;PSP_2bitTR	
;		ld a,%11001100 ; bits de fondo a 1
;PSP_confTR		
...
PSP_sp		; llamada con 1 solo parametro. ejemplo PRINTSP,31
		ld (PS2_spid), a; meto un 1 para que PRINTSPALL solo imprima 1. PS2_spid es un contador de sprites no es id
...
PSP_bgSprites:  ; V42 FUNCIONALIDAD SPRITES DE FONDO
		; aqui vamos a meter el chequeo de BC para ver si es un background sprite y en funcion del mode pondremos unos datos u otros
		; habra que hacerlo siempre, para todos los sprites porque todos pueden ser o no ser y hay que cambiarlo
		; OJO PRESERVAR BC		
		; voy a probar a meter basura en A y HL. Probado. no pasa nada
		;LD A,5
		;LD H,8
		;LD L, 6
		; to do
		...
		CALL BG_SPRITES; averigua si la imagen ubicada en BC es un bgsprite y en funcion del modo cambia operaciones binarias en las rutinas de impresion
...
PSP_CHECK_FLIP	; comprobacion flip
		...
		ld DE, _END_FLIP_IMAGES-1  ; v42
		ld HL,_BEGIN_FLIP_IMAGES-1; v42
		CALL CHECK_DOUBLE_LIMITS; v42
;		JR NC, PSP_noflip; v42
		JR C, hayflip; v42
		xor a
		JR PSP_noflip; v42
;---------------------------comentado en v42-------
...
;***********************************************************************
		...
		; creo que nos podriamos ahorrar estas 3 lineas y  funcionaria 
  		ld a, (PAT_from)
		and a
		Jr NZ,PSP_clipHder2
...
;		and a
		...
		ld BC, (SPR_image) ; imprescindible
		ld HL, _BEGIN_3D_SEGMENTS-1
		LD DE, _END_3D_SEGMENTS-1
		CALL CHECK_DOUBLE_LIMITS
		CALL C, _AJUSTA_SEGMENTO
;---------------comentado en v42---------------
; 		xor a
;		and a
;		ld (SPR_segment),a
...
PSP_clipHder2:
		...
		; caso 1 (el mas frecuente)= no hay cliping derecho
		ld hl, (SPR_coordx)
		ld de, (SPR_anchoaltofinal)
		ld d,0; e tiene el ancho
		add hl,de; en HL esta la coordenada superior derecha porque acabo de sumar el ancho
		ld bc, (SPR_MAX_X)
		;add a,0; para quitar acarreo. 
		and A; pone flag acarreo a cero
		SBC hl,bc; resta el limite. si queda dentro de la pantalla entonces saldra negativo
		JP M,PSP_cliphizq;-> no hay clipping derecho y nos vamos a comprobar el izquierdo

		; si se pasa del ancho no hay que imprimir
		ld c,l; l contiene lo que se ha salido de la pantalla, lo guardo en c
		sbc hl,de; d es cero. DE es el ancho porque E es el ancho. Es decir, vuelvo a la esquina sup izq del sprite
		ret nc; ojo si es cero tambien deberia retornar? no porque el limite es el primer byte que no se imprime. Es decir, hay clipping
		ret Z ; new *** es correcto

		; caso 2 y 3 ambos casos son de clipping. En un caso no hay nada que imprimir 
		; ojo hl es coordenadas. Ahora vamos a hablar de longitudes
		ld a,e;ancho esta en e
		sub c; c contiene lo que se ha salido de la pantalla. POr lo tanto A contiene ahora lo que queda dentro de la pantalla
		;comment ;ret M; nada que imprimir, esta todo fuera NO SE PUEDE DAR, ya se ha considerado antes
		;ret Z ; ESTE SI SE PUEDE DAR --> *** no, ya contemplado

		;si hay cliping y flip debo recolocar hl
		ld d, a ; ahora D contiene el ancho final, el trozo que se pinta
		ld a, (flip_flag)
		and a ; chequea si A es cero afectando a bit de signo.
		jr Z, PSP_noflipD
		ld hl, (SPR_imagefinal)
		ld b,0
	        add hl,bc
...
PSP_noflipD     ld a, d ; a tiene el ancho final ahora, ya recortado por cliping derecho

		ld de, (SPR_anchoaltofinal)
		ld e,a
		;ld (SPR_ancho),a ; *** new 5/1/2025 para permitir doble clipping
		ld (SPR_anchoaltofinal), de

		; si ha habido cliping izq, entonces no puede haberlo derecho.---> no se soporta doble clipping!
		;JR PSP_finclippling **** new 5/1/2025 tras el clip derecho comenzamos el izquierdo (ambos a la vez)
...
PSP_cliphizq:	
		;1er caso, el mas frecuente
  		ld HL, (SPR_coordx)
		ld de,(SPR_MIN_X); limite izquierdo establecido en SET_LIMITS
		;add a,0 ; desactivar el flag de acarreo
		and a; pone flag acarreo a cero. OJO, Si A es cero tambien activa flag de signo
		SBC HL,de ; HL=coordx - minx			
		...
		;----------------------------------------------------------------------
		;A creo que no puede ser cero aqui
;		xor a
		and a; pone flag acarreo a cero. OJO, Si A es cero tambien activa flag de signo 

		;ld a, (SPR_ancho); sin clipear
		ld a, (SPR_anchoaltofinal); ya clipeado por la derecha . Asi hacemos doble clipping. da igual!

		ld e,a
		ld d,0
;		add hl,de ; sin acarreo, NO VALE ADD porque no toca flags de signo ni zero ???
		adc hl,de
		;add hl,de
		ret M ; negativo. todo el sprite queda antes del limite. no hay que imprimir
		ret Z ; es necesario

		; si es positivo, hay algo que imprimir, y el nuevo ancho es hl (lo que traspasa el limite izquierdo)
		;--------------------------------------
		; si quiero calcular cuanto queda en el lado izq debo usar limitx-coordx
		ld de, (SPR_anchoaltofinal); ancho en e
		ld e, L
		ld (SPR_anchoaltofinal),de; ancho en e . ahora ya esta corregido el anchofinal
		

		; queda calcular el offset para la imagen, que es minx-coordx y meterlo en HL
		ld HL, (SPR_MIN_X)
		ld de,(SPR_coordx)
		and a; no deberia ser necesario. el acarreo deberia ser cero si llega hasta esta instruccion, pero si no lo pongo no sale bien
		sbc hl,de

		; BEGIN NUEVO 13/3/2025
;		ld a, (AUX_CLIPDOBLE); OJO QUE ESTO ES JUSTO LA RSTA CONTRARIA, QUIERO NEG (HL)
;neg 
;ld l,a
;ld h, 0
		; END  NUEVO 13/3/2025

;		ld a,(SPR_ancho); a contiene el ancho sin clipear!!
;		;ld a, (SPR_anchoaltofinal); ancho clipeado

		

;		sub l; le resta el ancho para saber cuanto queda fuera de la pantalla (se sale por la izq)
;		ld l,a
;		ld h,0; ahora hl contiene lo que queda fuera de la pantalla, el offset que deberemos darle a la imagen
		...
		ld a,(flip_flag)
		and a
		JR NZ, PSP_flipflag;v43
;		JR Z, PSP_restar
;		ld l,0 ; esto es imprescindible para imagenes flipeadas. sirve para lograr hl=0 

		
		; cambio la direccion de memoria del dibujo
PSP_restar	ld de, (SPR_imagefinal); es la direccion de la imagen (pasado ancho y alto)
		ADD hl,de
		ld (SPR_imagefinal),HL
		;cambiamos la coordenada x 
PSP_flipflag 	ld hl,(SPR_MIN_X)
  		ld (SPR_coordx),hl
...
PSP_DELETE	
;		nop ; codigo modificable dinamicamente para borrar v42_002
;		nop
;		nop
		; en caso de delete activo nos vamos a set_pnt_del
		ld a,(SPR_flag_delete)
		and a,1
		JP NZ,_SET_PNT_DEL
...
;		JR _PAINT;  ESTA INSTRUCCION NO HACE FALTA SI LO QUE SIGUE ES LA FUNCION _PAINT ; v42
		Jp _PAINT ;v42
		;ret
...
DIR_PRINTSPALL	JP _PRINT_SPRITES	;|PRINTSPALL, orden,animall,sync
DIR_COLSPALL	JP _COLSPALL		;|COLSPALL
DIR_AUTO_ALL	JP _AUTO_ALL		;|AUTOALL
DIR_MOVER_ALL	JP _MOVER_ALL		;|MOVERALL, dy,dx
DIR_MAP2SP	JP _MAP2SP		;|MAP2SP,yo,xo
DIR_STARS	JP _SCROLL_STARS	;|SCROLLSTARS, bank,num,color,dy,dx
DIR_COLAY	JP _COLISION_LAYOUT	;|COLAY, #,@colision
DIR_SETUPSP	JP _SETUP_SPRITE	;|SETUPSP, #, param_number, valor
DIR_LOCATESP	JP _LOCATE_SPRITE	;|LOCATESP, #,y,x
DIR_PEEK	JP _PEEK		;|PEEK,dir,@variable
DIR_POKE	JP _POKE		;|POKE,dir,@variable
DIR_ANIMASP	JP _ANIMA_SP		;|ANIMA, #
DIR_COLSP	JP _COLISION_SPRITES	;|COLSP, #,@id
DIR_RINK	JP _RINK		;|RINK,step
DIR_MOVER	JP _MOVER		;|MOVER, #,dy,dx
DIR_PRINTSP	JP _PRINT_SPRITE	;|PRINTSP, #,y,x
DIR_PRINTAT	JP _PRINTAT		;|PRINTAT,y,x,string
		;JP _DELSP		;|DELSP,spini, spfin
DIR_AUTO	JP _AUTO_SPRITE		;|AUTO,#
DIR_ROUTESP	JP _ROUTE_SP		;|ROUTESP,#
DIR_UMAP	JP _UPDATE_MAP		;|UMAP,dirmapini, dirmapfin,yini,yfin,xini,xfin
		;JP _ANIMA_ALL		;|ANIMALL				
DIR_ROUTEALL	JP _ROUTEALL		;|ROUTEALL
DIR_LAYOUT	JP _PRINT_LAYOUT	;|LAYOUT, y,x,string
		;JP _SETUP_SEQUENCE	;|SETUPSQ, #, adr0,adr1,...,adr6
DIR_MUSIC	JP _MUSIC_ON		;|MUSIC, loop_flag, cancion,speed
		;JP _MUSIC_OFF		;|MUSICOFF
DIR_SETLIMITS	JP _SET_LIMITS		;|SETLIMITS, xmin,xmax,ymin,ymax
DIR_3D		JP _3D_RENDER3D		;|3D, spritefin, mode
;		JP _NOP			;|NOP no es un comando, solo es para medir
; esto que viene a continuacion no es una rutina, sino una forma de almacenar una constante de ensamblaje
GET_IMAGE_LIST  DW		IMAGE_LIST; es una forma de almacenar la direccion de IMAGE_LIST siempre en el mismo lugar y es consultable
...
;=====================================
; variables de la funcion SALTO_INT
;======================================
...
PNT:
		ld de,(SPR_imagefinal); direccion de memoria donde se encuentra la imagen a pintar. alterado por cliping izq
		ld a, (SPR_ancho); ancho original de la imagen
		ld bc,(SPR_anchoaltofinal) ; ancho final en c . el ancho alterado por clip derecho
		sub c
		ld (PNT_restoancho),a; 
...
PNT_scanh
		JP PNT_NORMAL;IMPORTANTE - este salto es modificado en ejecucion justo despues del clipping
		; hay todos estos casos:
		;-----------------------
		;PNT_NORMAL
		;PNT_TRANSP
		;PNT_TRANSP_mode1	
		...
		;PNT_FLIP_TRANSP
		;PNT_FLIP_TRANSP_mode1
		;PNT_DEL (cubre mode 0, mode 1 y transparencias con 1 o 2 bit)


		;------------- NORMAL. Esto pinta una scanline del sprite
PNT_NORMAL
		ldir;  (DE)<--(HL) till bc=0
;		jr PNT_PAINTED
...
PTR2_scanh
		ld a, (HL); 2 pixels del sprite
		ld c, a; guarda  pixels de sprite en C		
PTR2_bin1	and a,%11000000; bit menos significativo de los 2 pixels del sprite. 
...
PTR2_bin2	cp %11000000; si los dos son 1, entonces no hacemos nada, no hay que reestablecer nada
		...
		res 6,c; lo pongo a cero
		ld a, (DE); leo la pantalla
PTR2_and	and a, %01010101; pixel derecho de la pantalla. cojo lo que tenga,incluido el fondo
...
; me llevo las variables a despues de 27000 v42
;sal_dir		dw 0; aqui guardamos la direccion
;sal_done	db 0; flag de direccion guardada
_SALTO_INT
		ld a,(sal_done)
...
; DE=end-1
; salidas
;  BC no se altera. HL si.
;  carry ON  dir  se encuentra en el intervalo
;  CARRY OFF dir no se encuentra en el intervalo
;--------------------------------------------------------
CHECK_DOUBLE_LIMITS
		and a; acarreo a cero
		sbc hl, bc  ; hl=begin, bc=dir . begin-dir si no desborda, then esta out
		ret nc ; nc significa que no esta dentro del intervalo
;		JR NC, CHECK_OUT
		ld l,c
		ld h,b
		; ahora hl es dir
		and a; acarreo a cero
		sbc hl,de  ; dir-end si no desborda esta fuera
;		jr nc, CHECK_OUT
...
ANS_a		;ld (ANS_id), a
		;ld c, a v3708
		; calculo de la direccion del sprite
ANS_again	call _get_dir_sp; A=num sprite, retorna HL con la direccion
		jr ans_sigue
...
ANS_nomacro
		...
		ld (ANS_newfradd), bc; la guardo en "new frame address"
		;ld HL,(ANS_newfradd)
		ld h,b
		ld l,c
...
ANS_nextframe	; aqui llegamos con la variable de nuevo frame id rellena y 
		...
		; el cero ya ha sido chequeado
		; 1=muerte , 2=fin,3..64 =cambio de seq,se pueden asignar tambien macroseq
		
		; nueva funcionalidad. si el valor de la direccion es 1 es que es una secuencia muerte
		ld HL, (ANS_newfradd)
		ld bc,255 ; en realidad podriamos poner un numero mucho mayor
		sbc HL,bc
		;JR NZ,ANS_nomuerte
		JR NC,ANS_normal ;v42

		; estamos aqui porque tenemos una de estas opciones  1=muerte,2=fin, N=nueva secuencia
		add hl,bc ; deshacemos la resta y ahora el registro L tiene el numero de frame
		ld a,l ; A contiene el frame id
		cp 1
		JR Z, ANS_muerte
		cp 2
		JR Z, ANS_fin
...
ANS_muerte	; si estamos aqui es secuencia muerte
		ld HL, (ANS_spaddress)
		ld (hl),0; status cerp
...
ANS_fin ; v42	; si estamos aqui habia un 2, lo cual es fin de secuencia
		ld HL, (ANS_spaddress)
		ld a, (hl)
		and a, %11110111; habia un #y # significa hexa (V43)
		ld (hl),A; status sin flag de animacion
		ret

ANS_otraseq;    si estamos aqui habia un N <64
		ld hl, (ANS_spaddress)
		ld bc,7 ; offset de la secuencia
		add hl,bc
;		ld (ANS_spaddress+7),a
		ld (HL),a
;		xor a
		inc hl
;		ld (ANS_spaddress+7),a
		ld (HL), 0 ; reset frame number
		;ld (ANS_newframeid), a
		;ret
		ld hl, (ANS_spaddress)
		Jp ANS_sigue; de este modo ya cambiamos a un frame de la nueva secuencia



ANS_normal	ld HL, (ANS_spaddress)
...
_check_image_list
		;--protecciones
		;push hl ; este deberia quitarlo (1/9/2024)
		push bc
		;
...
;		sbc hl,bc
;		JR NC, CIL_imagedir
		;--- esto tambien y es mas corto y rapido -------- (1/9/2024)
		ld a,b
		or a ; = cp 0
		JR NZ,CIL_imagedir ; si B=0 entonces BC<255 por lo tanto es un numero de la image_list
		;---------------
		; este bloque creo que sobra. los numeros [0..15] eran de uso futuro, creo. En ANIMA ya se pone la direccion al encontrar codigo final
		; he eliminado este bloque el 1/9/2024
		;pop hl; aqui tengo un numero # <255, ya que meti a bc
		;push hl
		;ld bc,16
		;and a
		;sbc hl,bc
		;JR C, CIL_imagedir ; si el numero es <16 considera que es una direccion
		; -----------------------
		...
		inc hl
		ld (hl),b
		dec hl ; asi me ahorro el push/pop de hl (1/9/2024)
		jr cil_end
...
; aqui estaba check_double_limits



;BG_SPRITES (sprites de fondo o "background" sprites) V42
;=========================================================
; llegamos aqui con
; BC = dir imagen
; GM_mode = modo de video
; BG_BITS = num bits background
;---------------------------------------------------
BG_BITS:	db 1 ; variable que almacena el num bits (default=1). no debe ir en segmento oculto pues un mode lo borraria

BG_SPRITES; v42
;JR KKKKKK
		
		ld HL, _BEGIN_BG_IMAGES-1
		ld DE, _END_BG_IMAGES-1
		CALL CHECK_DOUBLE_LIMITS
		;ld a,(GM_mode) ; antes del salto cargamos el mode para ahorrar instrucciones
		JR NC, BG_normal
...
;		ld de,_END_BG_IMAGES
;		sbc hl,de
; 	       ;JR NC, ZOM_fin;
;		ld a,(GM_mode) ; antes del salto cargamos el mode para ahorrar instrucciones
;		JR NC, BG_normal

BG_fondo	; la imagen es de fondo
		; modificamos transparencia. se fuerza
		ld a, (SPR_status) ; v42
		OR %01000000; v42
		ld (SPR_status),a; v42
		; ahora modificamos imagen
		ld a, (BC)
		inc bc
		ld h,a
		ld a, (BC)
		ld b,a
		ld c,h
		; necesitamos MODE y BG_BITS
		ld e, 0 ; en todos los casos de fondo se pone un cero
		ld a,(GM_mode) ;
		cp 1; considera que MODE esta en registro A
		JR NZ, BG_fondo_mode0


BG_fondo_mode1; en mode 1 solo puede haber 1 bit de fondo
		ld a,%00001111
		JR  BG_confTR	
...
BG_fondo_mode0
		ld a, (BG_BITS)
		cp 1
		JR NZ , BG_fondo_m0_2bits

BG_fondo_m0_1bits; imagen fondo en mode 0 con 1 bit fondo
		;ld a,%11000000
		ld a,%00111111
		JR  BG_confTR		
BG_fondo_m0_2bits; imagen fondo en mode 0 con 2 bit fondo
		;ld a,%11001100
		ld a,%00110011
		JR  BG_confTR		
...
BG_normal	; la imagen es normal
		ld a,(GM_mode)
		cp 1
		JR NZ, BG_normal_mode0

BG_normal_mode1 ; en mode 1 solo puede haber 1 bit de fondo
...
BG_normal_mode0; imagen normal, mode 0
		ld e,%11000000 ; con 1 o dos bits de fondo es lo mismo
		ld a, (BG_BITS)
		cp 1
		JR NZ , BG_normal_m0_2bits

BG_normal_m0_1bits; imagen normal en mode 0 con 1 bit fondo
		ld a,%11000000
		JR  BG_confTR		
BG_normal_m0_2bits; imagen normal en mode 0 con 2 bit fondo
		ld a,%11001100

		
BG_confTR	; configuracion de transparencia	

		ld (PTR2_bin3+1),a; sprites sin flip
		ld (PSTR_bin3+1),a; segmentos
		ld (PNTF_bin3+1),a; flip
		ld a,e
		ld (PTR2_bin1+1),a
		ld (PSTR_bin1+1),a
		ld (PNTF_bin1+1),a
...
;_SET_HEIGHT
;=============================================================================
; en el registro B llega el alto
; en SPR_Addr esta la dir del sprite, que es 27000 + id*16
; en _SPRITE_HEIGHTS_TABLE esta la lista de 32 bytes con las alturas de sprites
;
; por lo tanto
; la direccion en la tabla de alturas es
;  _SPRITE_HEIGHTS_TABLE+ (spr_addr -27000)/16 
;  como 27000/16 es 1687 entocnes
;  spr_addr/16 -1687 + _SPRITE_HEIGHTS_TABLE
;  spr_addr/16 +_SPRITE_HEIGHTS_TABLE -1687
...
if capacidad_layout  = 1 ; layout games
...
;__AAA
		...
		; V42_001 vamos a permitir que sean <255
		;-------------------------
		call _check_image_list ; *** esta es la unica linea nueva

		; ahora se resuelve la indireccion
		ld e,(hl)
		inc hl
		ld d, (Hl)
		ex de,hl
...
fill_noflip	ld h,d
		...
		ret
_end_fill_sp_data
__END_FREE_ZONE ; en assembling modo  cero esto es un hueco


;variables locales de la funcion de printsprite
PS2_flag_anim	db 0;
PS2_flag_sync	db 0;
PS2_orden	db 0; flag de ordenar y recorrer en orden
...
if ASSEMBLING_OPTION  = 0 
  ...
  org _END_27000_partial+1  ; en caso de opcion 0 , el map2sp se ensambla aqui
  _end_27000_continue
endif

if capacidad_scrolling =1 ; 
...
_MAP2SP
		...
		dec a ; opt
		JR NZ, M2S_no1
		; estamos aqui porque ha llegado un solo parametro (el status)
		ld a, (IX+0)
		ld hl, _MAP2SP_STATUS+1
		ld (hl),a
		; queda modificar la funcion de impresion de los segmentos 3d
		
		and 64 ; detecta bit transparencia en status
		JR NZ, M2S_PNTS_tr
	if capacidad_pseudo3d   =1
		ld hl, &B0ED ; &B0ED = LDIR
...
_MAP2SP_STATUS  ; con POKE a esta direccion+1 podemos modificar esto
		...
		;ld (HL),a
		ld (HL),%00000011 ; el estado por defecto es colisionable + printable
...
; creo que estas lineas sobran ya que van a ubicar mal la tabla sprite heights
;if ASSEMBLING_OPTION  = 0 
;_END_24000
;;ORG 30449 ; para continuar ensamblando el player en su sitio
;ORG 30388 
;endif
...
endif ; if capacidad scrolling



;=================================================
_FREE_ZONE; en assembling modos 0,1,2,3 es un hueco
...
; _SET_FLAG_DEL rutina que establece el flag de impresion con borrado (1=borrar o 0=imprimir)
;--------------------------------------------------------------------------------------------
;SPR_flag_delete db 0; a segmento 2
_SET_FLAG_DEL
	
	ld a,(IX+0)
	ld (SPR_flag_delete),a ; flag de borrado. 1= borrado activo. 0= borrado desactivado
	ret

; _SET_PNT_DEL rutina que establece la direccion de la subrutina de pintado para borrar
;--------------------------------------------------------------------------------------
_SET_PNT_DEL
	ld HL, PNT_DEL
	JP PSP_go_paint; cambia la rutina de pintar por HL y despues va a _PAINT

; rutina de borrado "basica"
;PNT_DEL
;	xor a
;	ld b,c
;BUC_DEL	LD (DE),a
...
; rutina de borrado "avanzada"

; la variable BG_BITS tiene el numero de bits de fondo
; la variable GM_mode tiene el modo de video
; mode 0
; 1 bit 11000000
; 2 bit 11001100
; mode 1
; 1 bit 11110000
; la idea es hacer and del pixel en pantalla con uno de esos numeros
; no es eficiente porque consultamos el status en cada scan. Esta rutina se ejecuta en cada scan
; lo mismo pasa con la mascara, se calcula en cada scan
PNT_DEL         ; rutina que pinta una scan en el modo delete
	
		ld a, (SPR_status);										
		and a, %01000000; flag de sobrescritura
		JR Z,DEL_NORMAL; gracias a la operacion AND, saltamos a DELETE NORMAL con A=0. 
	
	  	ld a, (GM_mode); asigno lo que hay dentro de GM_MODE
    		add a, a; A=Ax2
    		ld l,a; almaceno resultado parcial
    		ld a, (BG_BITS)
    		;dec a ; si quito el dec tendre que restar 1 a SPR_BG_MASK
    		add l; A = (GM_mode * 2) + (BG_BITS )
	
    		ld (DEL_ASIGNA+2),a ; modificacion dinamica del indice en la instruccion ld a, (ix + 0)
    		ld ix, SPR_BG_MASK -1; como he quitado el dec tendre que restar 1 a SPR_BG_MASK
DEL_ASIGNA
		ld a, (ix + 0)     ; el cero es un indice que lo hemos modificado

DEL_NORMAL 
 		; A ya es cero si venimos desde PNT_DEL y no hay owr (overwrite)
		ld b,c ; el ancho esta en bc, es decir en c
		ld l, a
BUC_DEL		LD a,(DE); leo lo que hay en la pantalla
		and l; contiene un cero o una mascara para restaurar el fondo
		ld (DE), a
		inc de
		djnz BUC_DEL; dec b hasta cero (b=ancho)
		Jp PNT_PAINTED
	

SPR_BG_MASK	db %11000000 ; modo 0, 1 bit
		db %11001100; modo 0, 2 bit
		db %11110000; modo 1, 1 bit
...
if ASSEMBLING_OPTION  = 4 
_END_30450_partial	db 0 ; este deb 0 no es nada, se va a machacar con el siguiente org
endif
...
ORG 30449 ; podemos poner esto aqui y asi el player va siempre en la misma direccion &76F1
_SPRITE_HEIGHTS_TABLE	ds 32 ; 32 bytes para almacenar las alturas de los sprites y poderlos ordenar. uso interno.

_BEFORE_PLAYER ;esta etiqueta debe ser anterior a &7711 = 30481