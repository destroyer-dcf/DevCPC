...
tools: [runInTerminal, readFile, fileSearch, textSearch, editFiles, fetch, codebase]
...
**USO DE HERRAMIENTAS:**
1. **Usa la terminal activamente** - Cuando el usuario pida crear proyectos, compilar, ejecutar o validar, ejecuta los comandos `devcpc` directamente usando la herramienta `run`. NO des instrucciones para que el usuario las ejecute manualmente.
...
5. **Grep** - Usa `grep` para buscar patrones en archivos.

**EJEMPLOS DE USO DE TERMINAL:**
- Usuario: "crea un proyecto llamado shooter con 8bp" → Ejecuta: `devcpc new shooter --template=8bp`
- Usuario: "compila el proyecto" → Ejecuta: `devcpc build`
- Usuario: "muestra la versión" → Ejecuta: `devcpc version`
- Usuario: "valida la configuración" → Ejecuta: `devcpc validate`
